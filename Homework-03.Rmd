...
```{r}
gmp <- gmp %>% mutate(pop = gmp/pcgmp, nlmfit = 6611*(gmp/pcgmp)^(1/8),nlmfit1=6611*(gmp/pcgmp)^(0.1),nlmfit15=6611*(gmp/pcgmp)^(0.15)) 
gmp %>% ggplot() + geom_point(aes(x = pop, y = pcgmp))+
  geom_line(aes(x = pop, y = nlmfit), col = 'blue', size = 1.5)+
  geom_line(aes(x =pop, y = nlmfit1),col='red',size=1.5)+
  geom_line(aes(x =pop, y = nlmfit15),col='yellow',size=1.5)
```
...
```{r}
plm=function(x,Y=gmp$pcgmp,N=gmp$pop){
  result=nlm(mse,x)
  b=list(y0=result$estimate[1],a=result$estimate[2],final_value=result$minimum)
  return(b)
}
```
...
answer:因为迭代的初值不同，所以最后收敛的结果也不同， $y_0 = 6611$ and $a = 0.15$MSE更低
```{r}
plm(c(6611,0.15))
plm(c(5000,0.1))
```
...
```{r}
n=length(gmp$pcgmp)
jackknifed.means=vector(length = n)
for (i in 1:n){
...
```{r}
plm.jackknife=function(x,Y=gmp$pcgmp){
  vec_y0=vector(length = n)
  vec_a=vector(length = n)
  for (i in 1:n){
    vec_y0[i]=plm(x,Y[-i])$y0[1]
    vec_a[i]=plm(x,Y[-i])$a[1]
  }
  result=list(y0_sd=sqrt((n-1)^2/n*var(vec_y0)),a_sd=sqrt((n-1)^2/n*var(vec_a)))
  return(result)
}
...
gmp_2013=gmp_2013%>%mutate(pop=round(gmp/pcgmp)) 
plm(c(6611,0.15),Y=gmp_2013$pcgmp,N=gmp_2013$pop)
plm.jackknife(c(6611,0.15),Y=gmp_2013$pcgmp)
```
